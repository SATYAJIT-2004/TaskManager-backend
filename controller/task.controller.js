...
export const getTasks = async (req, res, next) => {
  try {
    ...
    const pendingTasks = await Task.countDocuments({
      ...filter,
      status: "Pending",
      //If loggedIn user is not admin then add assignTo filter
      //If loggedIn user is admin then then nothing to do, just count
      ...(req.user.role !== "admin" && { assignedTo: req.user.id }),
    });
  ...
  try {
    ...
    if (req.body.assignedTo) {
      if (!Array.isArray(req.body.assignedTo)) {
        return next(400, "assignedTo must be an array of User IDs");
      }
...
export const userDashboardData = async (req, res, next) => {
  try {
    ...
    //Convert userId to objectId for preoper matching
